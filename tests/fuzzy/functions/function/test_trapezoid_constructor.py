...
def test_typical_trapezoid_membership_objects() -> None:
    ...
    t = TrapezoidFunction('test', 0., 1., 2., 3.)
    assert isinstance(t, TrapezoidFunction)
    t = TrapezoidFunction('test', -1., 0.1, 0.11, 5.)
    assert isinstance(t, TrapezoidFunction)


def test_infinite_trapezoid_membership_function_objects() -> None:
    t0 = InfiniteTrapezoidFunction('test', 0, 1)
...
def test_wrong_infinite_trapezoid_membership_function_objects() -> None:
    ...
    pytest.raises(AssertionError, TrapezoidFunction, 'test',
                  float('-inf'), 0., 1., 2)
    pytest.raises(AssertionError, TrapezoidFunction, 'test',
                  -1, 0., 1., float('inf'))
...
def test_wrong_vertices_values() -> None:
    pytest.raises(AssertionError, TrapezoidFunction, 'test', 0., -1., 2., 3.)
    pytest.raises(AssertionError, TrapezoidFunction, 'test', 0., 1., -1., 3.)
    pytest.raises(AssertionError, TrapezoidFunction, 'test', 0., 1., 2., -1.)

    pytest.raises(AssertionError, TrapezoidFunction, 'test', 0., 1., 0.5, 3.)
    pytest.raises(AssertionError, TrapezoidFunction, 'test', 0., 1., 2., 0.5)

    pytest.raises(AssertionError, TrapezoidFunction, 'test', 0., 1., 2., 1.5)
    pytest.raises(AssertionError, TrapezoidFunction, 'test', 0., 1., 2., -1.)

    pytest.raises(AssertionError, TrapezoidFunction, 'test', 0., 0., 0., 0.)


def test_triangular_constructor_good_values() -> None:
    # Sanity checks
    triangular_function = TriangularFunction('test', -100, 0, 10)
    assert isinstance(triangular_function, TrapezoidFunction)
    triangular_function = TriangularFunction('test', 0, 0.1, 1)
    assert isinstance(triangular_function, TrapezoidFunction)
    triangular_function = TriangularFunction('test', -2, -1, -0.2)
    assert isinstance(triangular_function, TrapezoidFunction)


def test_triangular_constructor_wrong_values() -> None:
    # Sanity checks
    pytest.raises(AssertionError, TriangularFunction, 'test', 0, -1, 2)
    pytest.raises(AssertionError, TriangularFunction, 'test', 0, 1, 0)
    pytest.raises(AssertionError, TriangularFunction, 'test', 0, 0, 0)
    pytest.raises(AssertionError, TriangularFunction, 'test', -1, -1, 2)
    pytest.raises(AssertionError, TriangularFunction, 'test', 0, 1, 1)
